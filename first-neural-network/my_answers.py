...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # Replace 0 your sigmoid calculation.
    ...
    def forward_pass_train(self, X):
        ...
        hidden_inputs = np.matmul(X.reshape(1, -1),
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y,
        ...
        hidden_error = np.matmul(self.weights_hidden_to_output,
                               output_error_term.reshape(-1, 1))

        hidden_error_term = hidden_error.T * hidden_outputs * (
            1-hidden_outputs)
    ...
    def run(self, features):
        ...
        hidden_inputs = np.matmul(np.array(features),
                                   self.weights_input_to_hidden)
        # signals into hidden layer
        hidden_outputs = self.activation_function(hidden_inputs)
        # signals from hidden layer
